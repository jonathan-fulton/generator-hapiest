...
`        super();
    ...
    static createFactoryClass(args) {
        ...
        const dependencies = [{variableName: args.associatedVoClassName, requireString: voFileLocationForRequire}];
        const methods = [];
        ...
        args.associatedVoProperties.forEach((property,index) => {
            ...
            newArgsBody +=
`            ${property.name}: ${assignmentValue}`;
            newArgsBody += (index < args.associatedVoProperties.length - 1) ? ",\n" : '';
        });
...
`       return ${args.factoryClassName}.createFromJsObj(newArgs);`;
        ...
        if (args.addCreateFromDbFunctions) {
            ...
            args.associatedVoProperties.forEach((property, index) => {
                ...
                const assignmentValue = `dbRow.${dbRowPropName}`;
                dbRowNewArgsBody +=
`            ${property.name}: ${assignmentValue}`;
                dbRowNewArgsBody += (index < args.associatedVoProperties.length - 1) ? ",\n" : '';
            });
